...
- **Pendente no Supabase**: `orders.cancellation_reason` (coluna nova) e `change_order_status` (parâmetro `p_reason`) ainda precisam ser aplicados no SQL Editor. **De propósito, não trava se `p_reason` vier nulo** — produção (`main`) ainda cancela sem mandar motivo nenhum; travar de verdade no RPC (`raise exception` se vazio) é o próximo passo, junto do release que leva essa tela pra produção, senão quebra o botão "Cancelar" de quem ainda roda o frontend antigo.
