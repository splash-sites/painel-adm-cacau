...
2. **Produtos e estoque** — CRUD em modal (não página cheia), estoque por loja, canal por tipo de consumo + revendedor, upload de foto, paginação configurável (10/20/50 por página, footer com "Linhas por página" + range + `‹ ›`). Tela "Organizar cardápio" (`/produtos/ordenar`) pra reordenar categoria e produto por drag (ver "Feature: Organizar cardápio"). Filtros server-side (estendem `ProductListParams`, nunca filtro no client): busca por texto (`ilike` em `external_code` OU `name`, com debounce 300ms e escape da sintaxe do `.or()` do PostgREST), Select de categoria (`category_id`), Select de tipo de cardápio (`available_dine_in`/`pickup`/`reseller`), mais o "Só incompletos" que já existia — todos combinam via AND e resetam pra página 1.
...
4. **Importação de planilha** — clicar "Importar planilha" já abre o seletor de arquivo do SO direto (sem tela intermediária pra escolher arquivo); depois de escolhido, cai direto no preview. Preview sempre antes de gravar. Ao lado, **"Exportar planilha"** baixa um `.xlsx` estilizado (cores do painel, cabeçalho congelado, nome da loja) com todos os produtos da loja ativa, nas **mesmas colunas que a importação lê** — dá pra editar e reimportar. Sem produto, sai só o cabeçalho.
...
**Exportar planilha** (botão ao lado de "Importar planilha", `ProductListPage.tsx`): `productRepository.listAll(storeId)` (loop de `.range()` em lotes de 1000) → `infrastructure/product/export/buildProductsWorkbook.ts` (ExcelJS, mesmo pacote do parser) monta o `.xlsx` estilizado com a paleta de marca (accent/creme/primary em ARGB), título com nome da loja nas linhas 1-2 (o parser acha o cabeçalho pela célula "Código", então título não atrapalha reimportação), cabeçalho congelado + autofilter, linhas alternadas, dinheiro com `numFmt`. Colunas idênticas às do import; `sim`/`não` pros booleanos; estoque 0 + `não` quando `track_stock=false`. `useExportProducts` (mutation em `useProducts.ts`) baixa via Blob + `<a download>` temporário (mesmo padrão de `exportReportCsv.ts`). Round-trip testado (`buildProductsWorkbook.test.ts`): exporta → `XlsxSpreadsheetParser` + `parseProductImportRow` reconstrói os campos.
...
**Bug real achado nesse redesign, corrigido na raiz (`ui/Label.tsx`)**: `Label`/`Input` "Da mesa"/"Até a mesa" saíram sobrepostos — reportado pelo usuário com print. Causa: `LabelPrimitive.Root` do Radix renderiza um `<label>` puro, que é `display: inline` por padrão do browser (Tailwind Preflight não reseta isso). Em todo outro form do app isso passava despercebido porque o `Input` ao lado é `w-full` — largura cheia força quebra de linha mesmo com o label inline, disfarçando o bug. Aqui os inputs são `w-20` (estreitos), então label+input cabem na mesma linha e colidem, expondo o problema de verdade. Fix: `Label` ganhou `block` na classe base — conserta pra sempre, não só nesse card, e não regride nenhum form existente (testado: `Dados da loja` com `w-full` e `ProductModal` com campos estreitos como "Valor não lover" continuam idênticos).
...
## Feature: Organizar cardápio (implementada)
Tela `/produtos/ordenar` (botão "Organizar cardápio" na toolbar de Produtos) pra ajustar a ordem do cardápio pela interface — antes só a coluna ORDEM da planilha escrevia `products.sort_order`. Espelha a estrutura do cardápio: seções por categoria (recolhidas por padrão, nome + contagem), produtos arrastáveis dentro de cada uma. **Sem SQL** — `products.sort_order` e `categories.sort_order` já existem.

**Decisões (perguntas fechadas):** renumeração **por categoria** (`0..n`, arrastar em "Bebidas" renumera só os produtos de Bebidas — os números repetem entre categorias, mas o cardápio nunca ordena produto entre categorias); **categoria e produto** arrastáveis na mesma tela (cabeçalho chama o `categoryRepository.reorder` que já existe); produtos sem categoria numa seção **"Sem categoria" no fim** (sem alça de categoria, sempre última); seções **recolhidas** ao abrir.

**Camadas:**
```
domain/product/groupByCategory.ts        -- groupByCategory(products, categories) + byProductOrder (sortOrder asc, nome desempata), puro, testado — mesma regra de ordenação que o storefront usa dentro da categoria
application/product/ProductRepository.ts  -- + reorderInCategory(storeId, categoryId: string|null, orderedIds)
infrastructure/.../SupabaseProductRepository.ts -- busca rows inteiros (select('*')) dos produtos da categoria (ou category_id is null) e upsert em lote com sort_order novo (row inteiro por causa das NOT NULL, mesmo cuidado de promotions/categories)
presentation/product/useProducts.ts       -- + useAllProducts (query ['products','all',storeId], reusa listAll) + useReorderProductsInCategory (otimista: mexe no cache ['products','all'], reverte no erro, invalida ['products'] no settled)
presentation/product/MenuOrderPage.tsx    -- tela; SortableList aninhado (categorias no de fora, produtos de cada seção no de dentro)
```
Carrega via `useCategoryList` + `useAllProducts`, agrupa com `groupByCategory` no client. Reorder de categoria reusa `useReorderCategories`.

**Storefront (fora daqui):** ordenar os produtos de cada categoria por `products.sort_order` (`public_products` já expõe a coluna). Se já faz isso, zero mudança — só passou a ter mais uma fonte escrevendo `sort_order` além da planilha.
...
**ALTO #1 — Dashboard sem fallback quando o Realtime cai (corrigido)**: `useOrderList` dependia 100% do canal `postgres_changes` pra saber que tinha pedido novo — se o WebSocket caísse (rede, aba suspensa, limite de conexão do projeto Supabase), o kanban parava de atualizar **em silêncio**, sem nenhum aviso. Dois fixes complementares:
1. `useOrderList` ganhou `refetchInterval: 30_000` — mesmo sem Realtime, o kanban se atualiza sozinho a cada 30s.
2. `OrderRepository.subscribeToStoreOrders` ganhou 3º parâmetro opcional `onStatusChange?: (status: RealtimeConnectionStatus) => void` (`'connected' | 'reconnecting'`), implementado no `.subscribe((status) => ...)` do Supabase Realtime (`SupabaseOrderRepository.ts`) — mapeia `'SUBSCRIBED'` pra `connected`, qualquer outro status (`TIMED_OUT`/`CLOSED`/`CHANNEL_ERROR`) pra `reconnecting`. `useOrderNotifications` (montado no `AppLayout`, roda em toda tela do painel) grava esse status num store Zustand novo e sem persist (`useRealtimeConnection.ts` — estado de conexão não deve sobreviver a um F5, reconecta do zero). `RealtimeConnectionBanner.tsx` (novo, mesmo estilo do `NotificationPermissionBanner`) mostra "Conexão em tempo real caiu — pedido novo pode levar até 30s pra aparecer aqui" só quando `status !== 'connected'`; estado inicial otimista (`'connected'`) evita flash falso-positivo no primeiro segundo de carregamento, antes do handshake confirmar.

**Testado ao vivo com queda real do canal** (não só leitura de código): `page.routeWebSocket(/realtime\/v1/, ws => ws.close(...))` do Playwright derruba a conexão WebSocket de verdade antes do app conectar — confirmado que o banner aparece (`.subscribe()` recebe o status não-`SUBSCRIBED` e propaga certinho pela cadeia store→componente) e que ele fica ausente em carregamento normal (conexão bem-sucedida, sem flash). `context.route()` (intercepta HTTP) tentado primeiro e **não funciona pra WebSocket** — Playwright trata upgrade de protocolo à parte, only `page.routeWebSocket`/`context.routeWebSocket` intercepta de verdade; registrado aqui pra não repetir a tentativa errada da próxima vez que precisar simular queda de canal.

**ALTO #2 — quantidade re-precificava adicional/variação com valor de hoje (corrigido)**: `handleQuantityBlur` (só quantidade, fora do modo "editar seleção") chamava o mesmo `update_order_item` do modo "editar seleção" — que faz **delete + insert** em `order_item_variations`/`order_item_addons`, regravando nome/preço/`price_mode` **atuais** das opções. Mexer só na quantidade de um item já pago/preparado silenciosamente puxava o preço do adicional/variação pro valor de hoje, quebrando o snapshot (princípio central do sistema, ver "Fluxo de status do pedido"). RPC nova, aditiva, sem tocar na `update_order_item` existente: `update_order_item_quantity(p_item_id, p_quantity)` só faz `update order_items set quantity = ...`, nunca chega perto de `order_item_variations`/`order_item_addons` — e reforça em Postgres o `canEditItems` (`status = 'received'`), mesmo padrão de sempre. `OrderRepository.updateItemQuantity` (novo, ao lado do `updateItem` que continua existindo pro modo "editar seleção") + `useUpdateOrderItemQuantity` + `OrderItemsEditModal.tsx` (`handleQuantityBlur` trocou de `updateItem.mutateAsync({ ..., selection: { ...reconstruído do snapshot } })` pra `updateItemQuantity.mutateAsync({ itemId, quantity })`, sem mais reconstruir `initialVariationOptionIds`/`initialAddons` pra mandar de volta pro RPC).

**Testado ao vivo com item real** (Fondue com variação "Banana" R$24,00 + adicionais "Chocolate preto" com "Bytes" +R$5,00 e "Granulado" +R$4,00, total R$33,00/unidade): mudou quantidade de 1→2 fora do modo de seleção, sem erro, card voltou mostrando "2× R$66,00" com variação e os 2 adicionais **exatamente iguais** (mesmo nome, mesmo preço) — confirma que a RPC nova não tocou nas linhas de seleção. Total da comanda (Mesa 12) atualizou de R$215,57 pra R$248,57 (delta R$33, 1 unidade a mais), confirmando consistência ponta a ponta. Quantidade restaurada pra 1 depois do teste, pedido de QA reaproveitado da suíte E2E já existente no projeto.

**ALTO #3 — Relatórios de 30 dias sem paginação, Postgrest truncava em silêncio (corrigido)**: `list()` (usado por Dashboard e por Relatórios via `useReportSummary`) e `listStatusHistory()` liam a página inteira num request só, sem `.range()` — o `max-rows` do Supabase (default **1000**) cortava o resultado sem erro nenhum, então faturamento/ticket médio/ranking de Relatórios (e "Tempo médio de preparo", que lê `order_status_history`, com 4-5 linhas por pedido) podiam ficar sub-contados em loja com bastante movimento em 30 dias, **sem nenhum aviso na tela**. Fix, mesmo princípio de `useAllStores()` (já usado pra Lojas): os dois métodos agora fazem loop com `.range()` num tamanho de página fixo (`LIST_PAGE_SIZE = 1000`, constante nova em `SupabaseOrderRepository.ts`), acumulando resultado até uma página vir com menos linhas que o tamanho pedido — contrato público (`Promise<Order[]>`/`Promise<OrderStatusHistoryEntry[]>`) não mudou, só parou de truncar. `listStatusHistory` ganhou `.order('changed_at')` explícito (não tinha nenhum antes) — paginação via `.range()` exige ordenação determinística, senão o Postgres não garante a mesma ordem entre uma chamada e outra e o loop podia pular ou repetir linha.

**Testado ao vivo**: Dashboard e Relatórios (`Hoje`/`30 dias`) carregam normal, sem erro de console, gráficos e KPIs renderizando (loja de teste tem bem menos que 1000 pedidos, então o loop sempre para na 1ª página — mesmo caminho de sempre, sem regressão). Não foi possível gerar >1000 pedidos reais pra forçar uma 2ª iteração do loop ao vivo; a garantia aqui vem da lógica (mesmo padrão já testado em `useAllStores`) + `npx tsc -b`/`npx vitest run` limpos, não de observação visual do loop em si.

**ALTO #4 — retry após falha no combo duplicava a promoção (corrigido)**: primeiro fix tentado (só capturar o id devolvido por `create()`/`update()`) **não resolvia o bug de verdade** — `create()` inseria a linha `promotions`, chamava `replaceComboItems` por dentro, e se esse passo falhasse a função inteira lançava erro *antes* de devolver o id, então `createdPromotion` nunca era setado e um reenvio chamava `create()` de novo (duplicava). Causa raiz: criar a promoção e salvar o combo estavam **bundlados num write só**, igual o bug original. Fix de verdade, espelhando `ProductModal` (que já resolve o mesmo problema pra vínculo de adicional/variação): separar em 2 passos client-side.
- `PromotionRepository.create()`/`update()` não chamam mais `replaceComboItems` internamente — só gravam os campos da promoção e devolvem o `Promotion` já com id.
- `replaceComboItems` virou público (`saveComboItems(promotionId, items)`, mesmo delete+insert de sempre) — passo separado, chamado só depois que o id da promoção já foi capturado.
- `PromotionModal.onSubmit`: 1º `savePromotion.mutateAsync` (grava a promoção, `setCreatedPromotion(savedPromotion)` assim que resolve) → 2º `saveComboItems.mutateAsync` (grava o combo). Se o 2º passo falhar, toast diz "Promoção salva, mas falha ao salvar combo — revise abaixo e salve de novo" e o modal continua aberto **com `createdPromotion` já setado** — reenviar vira `update()`, nunca cria uma segunda linha.

**Testado ao vivo forçando a falha de verdade** (não só leitura de código): `page.route('**/rest/v1/promotion_combo_items*', ...)` do Playwright intercepta e derruba com 500 só a 1ª tentativa de INSERT do combo — confirmado 1º submit mostra o toast de falha parcial, reenviar (clicar "Salvar" de novo no mesmo modal) mostra "Promoção criada." e **só existe 1 linha** com o título de teste na lista (nunca 2). Prova que a 1ª versão do fix (só capturar o id no retorno) não bastava e a separação em 2 passos era necessária de verdade — achado no próprio processo de testar ao vivo, não hipotético. Promoção de teste excluída depois do teste.

**ALTO #5 — `xlsx@0.18.5` com CVE sem correção publicada no npm (corrigido)**: SheetJS free (`xlsx`) tem prototype pollution (GHSA-4r6h-8v6p-xvw6) e ReDoS (GHSA-5pgg-2g8v-p4x9) — a versão corrigida (0.20.2+) não é mais publicada no `npm`, só distribuída via CDN próprio, fora do registro padrão. Escolha (perguntado ao usuário, não decidido sozinho): trocar por **`exceljs`** — mantida ativamente, sem CVE conhecido, 100% via npm — em vez do tarball do CDN do SheetJS (mesma API de hoje, mas dependência instalada fora do npm normal). Trade-off aceito conscientemente: `exceljs` só lê `.xlsx` moderno (OOXML), **não lê `.xls` antigo** (Excel 97-2003, formato binário) — `XlsxSpreadsheetParser.ts` agora detecta esse caso e lança erro explícito orientando salvar como `.xlsx`/`.csv`, em vez de travar silencioso; `accept` do input de arquivo (`ProductListPage.tsx`/`ProductImportPage.tsx`) removeu `.xls` da lista pra não nem oferecer a opção. CSV nunca foi responsabilidade do `exceljs` (só lê o formato OOXML) — parser CSV próprio, mínimo, escrito do zero: suporta campo entre aspas (delimitador/quebra de linha dentro do campo, `""` como aspas escapada) e **detecta automaticamente `,` vs `;`** contando ocorrências na 1ª linha (Excel em pt-BR exporta CSV com `;` por padrão, já que `,` é separador decimal nesse locale — regressão real que teria acontecido se eu hardcoasse vírgula).
...
**CRUDs testados ponta a ponta (criar → editar → excluir, ou criar → editar → verificar quando exclusão não se aplica), todos OK, zero erro de console**: Produtos, Categorias, Adicionais (grupo + opção), Variações (grupo + opção), Atendentes, Usuários (`/usuarios`, incluindo as 2 Edge Functions de criar/editar/excluir admin), Lojas (`/lojas/nova`, incluindo excluir loja de teste depois — zona de risco funcionando), Configurações (update de dados da loja), Promoções (load, CRUD completo já validado em rodada anterior desta sessão). Histórico e Relatórios (leitura, todos os filtros de período) carregam sem erro.

**2 bugs reais de performance achados e corrigidos** (achados só possíveis medindo requisição de verdade, não por leitura de código):

**Bug 1 — `useAuth()` criava um listener de Realtime auth por componente que o chama, não um só pro app inteiro.** `useAuth.ts` tinha a leitura (`useQuery`) **e** a assinatura (`useEffect` + `onAuthStateChange`) no mesmo hook — como 8 componentes diferentes chamam `useAuth()` (`AppLayout`, `ProtectedRoute`, `UserMenu`, `useEffectiveStoreId` — usado em quase toda tela — `SettingsPage`, `AdminUserListPage`, `EditAdminProfileModal`, `LoginPage`), existiam **8 assinaturas independentes** de `supabase.auth.onAuthStateChange` ao mesmo tempo. Cada evento de auth (login, refresh de token, foco de aba) disparava 8 fetches independentes de `profiles` — confirmado ao vivo: **19-21 chamadas a `/rest/v1/profiles` só pra carregar 1 tela**. Fix: `useAuth()` virou hook só de leitura; a assinatura foi extraída pra `AuthSessionSync.tsx`, componente novo montado **1x** em `App.tsx` (fora de qualquer rota), que escreve no mesmo cache do React Query (`queryClient.setQueryData`). Testado ao vivo: chamadas de `/profiles` caíram de ~20 pra **4** na mesma tela, sem quebrar login nem logout (os dois retestados explicitamente após o fix).

**Bug 2 — CRÍTICO — loop de refetch infinito em Relatórios enquanto a tela ficasse aberta.** `useReportSummary.ts` calculava `since = sinceIsoForPeriod(period)` **sem memoizar**, e `sinceIsoForPeriod` usa `new Date()` (precisão de milissegundo) pros períodos `7d`/`30d`. Esse `since` alimentava direto a `queryKey` de `report-status-history` (`['report-status-history', storeId, since]`) e de `report-preceding-phones` — como o valor mudava a cada render (mesmo com `storeId`/`period` idênticos), o React Query via cada render como uma query **nova**, disparava fetch na hora, a resposta causava novo render, que recalculava `since` de novo (outro valor), nova query "nova", novo fetch — **loop autossustentado, sem fim enquanto o usuário deixasse a aba de Relatórios aberta**, só limitado pela latência de rede (~100-170ms por volta). Confirmado ao vivo instrumentando o hook: **30-52 renders/refetches em 2.5 segundos** depois de 1 clique só numa aba de período; com 10 lojistas com Relatórios aberto ao mesmo tempo, isso vira um flood contínuo no banco, não um pico pontual. **1ª tentativa de fix não resolveu** (achei que era o mesmo bug 1 se propagando via `useEffectiveStoreId`, mas o `storeId` provou estar 100% estável nos logs — descartado por instrumentação direta, não suposição). Causa real, achada só depois de isolar com `console.log` dentro do próprio hook: `since`/`currentPeriodPhones` (o array de telefones do período, também recalculado sem memo) nunca deveriam ter entrado cru nas `queryKey`. Fix: os dois viraram `useMemo` (`since` depende só de `[period]`, `currentPeriodPhones` só de `[orders]`) — mesmo valor entre renders enquanto o período não mudar de verdade, key do React Query volta a ser estável. Testado ao vivo: de 16-17 chamadas por clique de aba pra **1-2** (o esperado), telas renderizando idênticas (KPIs, gráficos, sem regressão visual).

**Achado à parte, não é bug de código**: uma promoção real da loja ("TESTE - Fondue com desconto") tem `image_url` apontando pro bucket de OUTRA loja — confirmado via REST direto que a query de `promotions` filtra certo por `store_id` (não é vazamento de RLS/query), é lixo de dado de teste de sessão anterior misturado com produção. Fica registrado aqui, não corrigido (não é decisão de código, é limpeza de dado — usuário decide se quer apagar/reupar a imagem certa).

**Balanço pra "10 usuários simultâneos"**: os 2 bugs corrigidos eram os únicos amplificadores de verdade encontrados na varredura — o resto do app (CRUDs, Dashboard com o polling/fallback do ALTO#1, paginação em loop do ALTO#3) já se comporta de forma limitada e previsível por usuário. Os itens MÉDIO/BAIXO da auditoria anterior (N+1 em seletor de variação/adicional, busca de produto sem índice trigram, etc. — ver seção de auditoria acima) continuam pendentes e valem a pena revisitar, mas nenhum tinha a gravidade de um loop infinito — esses dois eram, de longe, os que mais importavam pra concorrência real.
